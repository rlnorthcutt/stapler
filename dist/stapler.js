...
`;
  ...
  var Stapler = class extends HTMLElement {
    ...
    _detectMode() {
      ...
      if (attr === "flow") {
        const hasBreaks2 = this.querySelector("page-spacer") !== null;
    ...
    _processPageBreaks(wrapper, slotH, pageH, headerH, footerH) {
      const breaks = Array.from(wrapper.querySelectorAll("page-spacer"));
    ...
    _teardownFlow() {
      ...
      if (wrapper) {
        ...
        wrapper.querySelectorAll("page-spacer").forEach((pb) => {
          pb.style.height = "";
        });
  ...
  // src/components/PageSpacer.ts
  var PageSpacer = class extends HTMLElement {
    static {
      this.TAG = "page-spacer";
    }
  };
